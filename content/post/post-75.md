...
先说清楚，Skill 和 Agent 是两个东西，不是一种技术。Skill（技能）和 Agent（智能体）经常被捏在一起说，但它们是两个独立的概念、两层独立的资产。
...
再看是什么。数据集是训练、微调、评测模型的数据资产：指令对（问题-标准答案）、对话对（多轮对话）、评测集（有标准答案的验收用例）。数据集管理平台负责数据的采集、清洗、标注、版本管理、评测。我做的数据集管理服务就是这个方向——它本质上是数据资产的仓库和流水线。
