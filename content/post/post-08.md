...
三个关键点：外部传进来的不是"把状态改成 PAID"，而是一个领域事件，比如 PaySucceeded，能不能迁由聚合根自己判断；状态机触发的副作用，账户流水、账单生成、消息发送，要么同事务落库，要么走 Outbox，不能裸调；充值、消费、退款多类型订单复用同一张状态图，差异靠 OrderType 决定允许的事件子集和后置处理器。
...
状态机的价值不在代码精巧，在于把业务规则从散落各处的 if 收敛到一个看得见的地方。后来接入退款、分账、跨境支付，我们都是先在状态图上画好新状态和新事件，再动手写代码，这套顺序帮团队躲开了不少状态错乱的坑。
