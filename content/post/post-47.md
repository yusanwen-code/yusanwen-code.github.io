...
- 重试：只对明确可重试的错误（429、500、502、503、连接错误、EOF）做指数退避重试；400/401/403 这类业务错误立即抛。流式请求已经吐过 token 的不重试——重试会让前端重复看到字。
...
这套东西上线之后，任何一家供应商出问题，知识库问答服务都能"带病工作"，业务侧基本无感。回头看，LLM 集成真正的稳定性问题，绝大多数不在模型本身，而在它周边的网络和配额。
