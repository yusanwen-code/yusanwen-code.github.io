...
知识库问答服务要同时对接 OpenAI、Azure、VLLM、HuggingFace 以及一堆国内供应商（DeepSeek、通义、智谱、Kimi、百川、文心），再加上私有化部署的 Ollama。每家的配额、QPS、TPM、错误码、重试语义都不一样。上线初期我们踩了一堆坑：某家供应商偶发 429，我们无脑重试把情况搞得更糟；另一家流式接口断连，整条对话直接挂掉；还有一次某供应商整体不可用，全站问答跟着挂。
