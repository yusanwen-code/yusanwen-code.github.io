...
我们的解法是引入 StarRocks 做分析型数仓，MySQL 继续存权威交易数据，组成多数据源架构。真正要回答的是三个问题：代码里怎么清晰地路由查询，数据怎么从 MySQL 同步到 StarRocks，配置怎么统一管。
...
- 别在 StarRocks 上做高频点查和单行更新。它是批量向量化引擎，高并发小查询性能反而不如 MySQL，主键模型的 Update 也是异步合并的，不适合交易场景。两类负载，严格分开。
...
MySQL + StarRocks 的组合，本质就是让专业的引擎干专业的事：MySQL 扛交易，StarRocks 扛分析。这套架构里真正起作用的，不是「自动路由」这种花哨功能，而是职责划分清楚、依赖显式注入、同步链路可靠，以及对数据延迟坦诚。配置收口之后，新增一个分析查询就是写一个走 StarRocks DAO 的方法，和在线业务互不干扰。
