...
知识库问答服务上线初期只做单轮知识库问答：用户提问 → 检索 → 拼 Prompt → 调 LLM → 返回答案。这个流程简单可控，但很快就遇到瓶颈。用户问的问题越来越复杂："帮我查一下上个月引用量最高的三篇文献，对比它们的方法，再导出成 Word"——这一句话里包含检索、排序、对比分析、文档生成四步，单轮 RAG 根本接不住。
