...
在某科技公司主导 AI 数据平台后端架构时，我们有 统一支付平台 支付、统一认证中心 认证、数据治理服务、数据集管理服务、知识库问答服务 等多个服务，早期部署在虚拟机上靠 Shell 脚本 + Docker Compose 管理。每次发布需要 SSH 到各台机器拉镜像、重启容器，流程繁琐且容易出错，回滚更是靠手速。发布频率高的时候，运维和开发都苦不堪言。
...
- **灰度发布**：KubeSphere 内置的金丝雀发布基于 Istio，第一次接入时 Sidecar 注入导致请求延迟增加约 20ms，对 知识库问答服务 的流式响应影响不大，但 统一支付平台 支付场景需要注意。
