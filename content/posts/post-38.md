...
知识库问答服务 最早的工具调用是把工具列表拼在 system prompt 里让模型自己输出 JSON，结果模型经常漏字段、把枚举值写错、甚至输出一段自然语言而不是 JSON。OpenAI 兼容协议都支持 Function Calling 后，我们切换到原生 tool_calls，但生产环境又冒出新问题：模型选错工具、参数不合法、工具执行超时、多轮调用陷入死循环。
...
Function Calling 的工程化重点不在"调通接口"，而在围绕模型不确定性做防御：schema 校验、执行隔离、迭代上限、结果裁剪。知识库问答服务 用统一的 Tool 接口和执行循环把这些横切逻辑收敛到框架里，新增业务工具只需要实现接口、写好描述，就能安全地交给模型调用。
