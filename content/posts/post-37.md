...
数据集管理服务 承接了大量 PDF/Word 学术文档的解析与向量化，服务于 知识库问答服务 的知识问答。早期我们图省事，文档解析出纯文本后直接按 500 字符定长切分，效果很不稳定：表格被腰斩、段落中间被切开、跨页的章节标题和正文分到了不同块。召回时经常出现"答案明明在文档里但没召回到"的情况。
...
没有万能分块尺寸，分块策略要跟着文档类型走。结构化文档优先用结构信息，纯文本用递归字符分块兜底，表格单独成块。在 数据集管理服务 里我们按文档类型路由 Chunker，并把分块结果和 metadata 一起入库，知识库问答服务 的问答命中率比定长切分有明显改善。
