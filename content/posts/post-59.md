...
在 知识库问答服务 项目里，我们最初给企业客户定制 Agent，就是改一段 system prompt：开头写"你是一个 XX 助手"，中间堆一堆语气要求，结尾补几条禁忌。结果维护起来非常痛苦：客户 A 要"学术严谨"，客户 B 要"活泼亲切"，两份 prompt 八成内容重复，改一处公共措辞要同步十几份；更糟的是，prompt 写长了之后，模型对后半段指令的遵循度明显下降，"必须用 markdown 表格回答"和"回答控制在三句话"这种约束经常被忽略。
...
**few-shot 存哪里**。示例对话可能很长，全塞 MySQL 的 JSON 字段会让行膨胀。我的做法是短示例直接存库，长附件（比如整段对话日志）存 S3，Elixir 里只留 `few_shot_refs` 引用，渲染前由引擎批量拉取。这跟我在 数据治理服务 里做 S3 预签名上传是同一套思路。
