...
**缓存击穿**。某个热门分身的缓存恰好过期，瞬间大量请求都未命中，全打到 Python 引擎和 LLM。我用 `singleflight`（Go 侧）+ Redis `SetNX` 双保险：同一时刻同一个 lineage_hash 只有一个请求真正去合成，其余等结果。这跟我在 统一认证中心 里做 JWT 刷新防雪崩是同一个套路。
