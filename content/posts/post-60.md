...
**交叉不是简单拼接**。第一版我直接把两个金丹的 principles 列表 concat，结果 prompt 里出现自相矛盾的两条原则。后来加上 `dedup_merge`——先用 embedding 相似度去重（我在 数据集管理服务 里做向量化那套直接复用），再让 LLM 对剩余冲突逐条裁决。去重这一步很关键，它能把两个"意思一样但措辞不同"的原则合并掉。
